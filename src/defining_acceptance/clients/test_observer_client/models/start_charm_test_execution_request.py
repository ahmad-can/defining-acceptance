...
from ..models.charm_stage import CharmStage
...
T = TypeVar("T", bound="StartCharmTestExecutionRequest")


@_attrs_define
class StartCharmTestExecutionRequest:
    """
    Attributes:
        ...
        family (Literal['charm']):
        revision (int): Charm revision number from Charmhub. Each architecture build has its own revision. Find this via
            'charm info <charm-name>' or in Charmhub.
        track (str): Charm release track being tested. Tracks represent different major versions or development streams.
            Common values: 'latest' (default), version-based tracks like '1.0', '22'. Use 'latest' if unsure.
        ...
        branch (str | Unset): Optional charm branch name within the track/risk level. Branches are temporary testing
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        family = cast(Literal["charm"], d.pop("family"))
        if family != "charm":
            raise ValueError(f"family must match const 'charm', got '{family}'")

        revision = d.pop("revision")

        track = d.pop("track")

        execution_stage = CharmStage(d.pop("execution_stage"))
        ...
        start_charm_test_execution_request = cls(
            name=name,
            version=version,
            arch=arch,
            environment=environment,
        ...
        start_charm_test_execution_request.additional_properties = d
        return start_charm_test_execution_request
